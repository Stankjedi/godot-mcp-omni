...
function getTextContent(mcpResult: any): string {
  if (!mcpResult || !Array.isArray(mcpResult.content)) {
    throw new Error(`Bad MCP result: ${JSON.stringify(mcpResult)}`);
  }
  const text = mcpResult.content.find((c: any) => c?.type === 'text')?.text;
  if (typeof text !== 'string') {
    throw new Error(`Missing text content: ${JSON.stringify(mcpResult)}`);
  }
  return text;
}
...
export class JsonRpcProcessClient {
  ...
  constructor(private child: ChildProcessWithoutNullStreams) {
    ...
    this.child.on('exit', (code) => this.failAll(new Error(`Server exited (code=${code ?? 'null'})`)));
    this.child.on('error', (err) => this.failAll(new Error(`Server error: ${String(err)}`)));
  }
  ...
  async send(method: string, params: unknown, timeoutMs = 30000): Promise<JsonRpcResponse> {
    if (this.closed) throw new Error('JSON-RPC client is closed');
    const id = this.nextId++;
    const request = { jsonrpc: '2.0', id, method, params };

    if (!this.child.stdin.writable) throw new Error('Server stdin not writable');
  ...
  async callTool(name: string, args: Record<string, unknown>, timeoutMs = 30000): Promise<any> {
  ...
  async callToolOrThrow(name: string, args: Record<string, unknown>, timeoutMs = 30000): Promise<any> {
